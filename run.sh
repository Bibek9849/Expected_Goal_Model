...
if [ $DATA = "True" ]
then
    python3 -m src.dataset $TYPE                   ## create dataset
    
    echo "Creating train-test data..."
    python3 -m src.train_test_data $TYPE               ## create train and test data
fi
...
then
    ...
    python3 -m src.categorical $TYPE $ENC_TYPE         ## categorical encoding

    echo "Preparing dataset for training and prediction..."
    python3 -m src.prepare_dataset $TYPE $ENC_TYPE     ## prepare dataset for training and prediction
fi
...
python3 -m src.train                               ## train, predict and save
